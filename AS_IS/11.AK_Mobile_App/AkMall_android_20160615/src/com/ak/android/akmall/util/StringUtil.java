...
public class StringUtil implements java.io.Serializable {
	...
	public static String vectorToStringWithAmp(Vector<String> vector) {
		String str = new String();
		if (vector != null) {
	...
	public static String arrayListToStringWithAmp(List<String> list) {
	...
	public static String substring(String src, int start, int end) {
		if (src == null || "".equals(src) || start > src.length()
				|| start > end || start < 0)
			return "";
		if (end > src.length())
			end = src.length();
	...
	public static boolean checkString(String str)
		...
		for (int i = 0; i < a.length; i++) {
			int num = a[i];
			if ((num == 95) || (num >= 97 && num <= 122)
					|| (num >= 65 && num <= 90) || (num >= 48 && num <= 57)) {
	...
	public static String replaceAll(String str, String changStr, String toStr) {
		...
		if (str == null || str.equals(""))
			return result;
		if (str.indexOf(changStr) == -1)
			return str;
		result = str;
		while (true) {
			result = replace(result, changStr, toStr);
			if (result.indexOf(changStr) == -1)
				break;
		}
		return result;
	}
